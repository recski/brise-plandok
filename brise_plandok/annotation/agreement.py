...
def eval_against_gold(data, attr_vocab, annotator_vocab):
    if "gold" not in annotator_vocab.word_to_id:
        logging.info("No gold data is provided - skipping evaluation against gold")
        return
...
def measure_agreement(data, attr_vocab, annotator_vocab):
    ...
    ratings = np.zeros((len(attr_vocab), len(annotator_vocab), len(data)), dtype="int")
...
def print_iaa(stats):
    ...
    for (
        ...
        gold_kappas_str = (
            " ".join(f"{k:.2f}" for k in gold_kappas) if gold_kappas else "N/A"
        )  # noqa
...
def _print_disagree(df, disagree):
    logging.info(
        f"Disagree: {disagree.shape[0]}/{df.shape[0]} - {int(disagree.shape[0]/df.shape[0]*100)}%\n{disagree}"
    )
...
def load_data(filenames):
    ...
    for fn in filenames:
        ...
        for sen in gen_sens_from_file(fn, filetype):
            ...
            if sen["id"] in data:
                assert (
                    data[sen["id"]]["text"] == sen["text"]
                ), f'changed text in {fn} sentence {sen["id"]}'  # noqa
            else:
                data[sen["id"]] = {
                    "id": sen["id"],
                    "text": sen["text"],
...
def gen_sens_from_xlsx(fn):
    for i, row in enumerate(xlsx_to_data(fn)):
        ...
        attributes = [
            field
            for j, field in enumerate(rest)
            if field and j % 2 == 1 and field not in ATTR_IGNORE
        ]

        yield {"id": sen_id, "text": sen_text, "attributes": sorted(attributes)}


def print_data(data, attr_vocab, annotator_vocab, out_fn):
    cols = (
        ["sen_id", "sen"]
        + list(annotator_vocab.word_to_id.keys())
        ...
        df = df.append(
            pandas.DataFrame(
                [
                    [sen["id"], sen["text"]]
                    + [
                        ",".join(attr_vocab.get_word(attr) for attr in attrs)
                        if attrs
                        else "no_attr"
    ...
    disagree = df[df["full_agreement"] is False].iloc[:, [0]]
    _print_disagree(df, disagree)
...
def _get_attr_by_annotator(attr_vocab, sen):
    return [
        ",".join(attr_vocab.get_word(attr) for attr in attrs) if attrs else "no_attr"
        for ann, attrs in sen["annot"].items()
    ]
...
def main():
    ...
    if len(filenames) < 2:
        logging.error("At least two annotation files are required.")
        return
