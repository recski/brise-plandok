            ...
            if sentence_ids[i].split("_")[-2] == "0":
                ...
                if len(labels) > 0 and labels[0]:
                    self.__set_attributes(
                        sheet_obj, self._get_coordinate("C", row_id), self._get_coordinate("D", row_id), labels[0], attribute_to_attribute_class)

                if len(labels) > 1:
                    self.__set_attributes(
                        sheet_obj, self._get_coordinate("E", row_id), self._get_coordinate("F", row_id), labels[1], attribute_to_attribute_class)

                if len(labels) > 2:
                    self.__set_attributes(
                        sheet_obj, self._get_coordinate("G", row_id), self._get_coordinate("H", row_id), labels[2], attribute_to_attribute_class)

                if len(labels) > 3:
                    self.__set_attributes(
                        sheet_obj, self._get_coordinate("I", row_id), self._get_coordinate("J", row_id), labels[3], attribute_to_attribute_class)

                if len(labels) > 4:
                    self.__set_attributes(
                        sheet_obj, self._get_coordinate("K", row_id), self._get_coordinate("L", row_id), labels[4], attribute_to_attribute_class)
