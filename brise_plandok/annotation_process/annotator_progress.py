...
class ProgressTacker:
    ...
    def __init__(self, phase):
        ...
        if phase == 1:
            ...
            raise ValueError(f"Phase must be 1 or 2")

    def get_annotator_progress(self, ann_folder):
        uploaded_dict = {}
        for annotator in ANNOTATORS:
            df = load_assigned_docs_as_df(ann_folder, annotator, self.phase)
            uploaded = self._get_uploaded_docs(os.path.join(
                ann_folder, annotator, "phase"+str(self.phase), ANNOTATOR_UPLOAD_FOLDER))
    ...
    def get_ready_docs(self, doc_tracking_file, uploaded_dict, first, last, only_yes, only_new):
        ...
        once_annotated = df[(df[ANNOTATOR_HEADERS[-2]] == True)
                            | (df[ANNOTATOR_HEADERS[-1]] == True)].shape[0]
...
def main():
    ...
    if args.doc_tracking:
        progress_tracker.get_ready_docs(
            args.doc_tracking, uploaded, args.first, args.last, args.only_yes, args.only_new)
