...
class FullAnnotationPreFiller:
    ...
    def generate_for_full_annotation(self, doc_ids, data_folder):
        sen_to_full_gold_attrs = SenToAttrMap(
            gold_dir=data_folder, fuzzy=True, full=True) if data_folder else None
        sen_to_gold_attrs = SenToAttrMap(
            gold_dir=data_folder, fuzzy=True, full=False) if data_folder else None
