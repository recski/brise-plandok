...
class FullReviewExcelGenerator(ExcelGenerator):
    ...
    def _modify_header(self, sheet, doc):
        if len(doc[DocumentFields.ANNOTATORS]) > 0:
            sheet.cell(
                row=1, column=self.CONSTANTS.MODALITY_ANN_1_COL).value = doc[DocumentFields.ANNOTATORS][0]
        if len(doc[DocumentFields.ANNOTATORS]) > 1:
            sheet.cell(
                row=1, column=self.CONSTANTS.MODALITY_ANN_2_COL).value = doc[DocumentFields.ANNOTATORS][1]
        for col in range(self.CONSTANTS.ATTRIBUTE_OFFSET, column_index_from_string(coordinate_from_string(self.CONSTANTS.LAST_COLUMN)[0]), self.CONSTANTS.ATTRIBUTE_STEP):
            if len(doc[DocumentFields.ANNOTATORS]) > 0:
                sheet.cell(
                    row=1, column=col+self.CONSTANTS.TYPE_ANN_1_OFFSET).value = doc[DocumentFields.ANNOTATORS][0]
            if len(doc[DocumentFields.ANNOTATORS]) > 1:
                sheet.cell(
                    row=1, column=col+self.CONSTANTS.TYPE_ANN_2_OFFSET).value = doc[DocumentFields.ANNOTATORS][1]
