...
class LabelReviewExcelGenerator(ExcelGenerator):
    ...
    def _gen_attributes(self, sen):
        return [
            {AttributeFields.NAME: attr}
            for attr in list(sen[SF.ANNOTATED_ATTRIBUTES].keys())
            + list(sen[SF.GOLD_ATTRIBUTES].keys())
        ]
    ...
    def _fill_attribute(self, attribute, sen, sheet, col, row):
        ...
        sheet.cell(row=row, column=col + self.CONSTANTS.LABEL_OFFSET).value = attribute_name
    ...
    def __set_annotator_count(self, attribute_name, sen, sheet, row, col):
        ...
        if attribute_name not in sen[SF.ANNOTATED_ATTRIBUTES]:
            ...
            count = len(sen[SF.ANNOTATED_ATTRIBUTES][attribute_name][AAF.ANNOTATORS])
        sheet.cell(row=row, column=col + self.CONSTANTS.COUNT_OFFSET).value = count
    ...
    def __set_annotator_text(self, attribute_name, sen, sheet, row, col):
        ...
        sheet.cell(row=row, column=col + self.CONSTANTS.ANNOTATORS_OFFSET).value = annotators

    def __set_review_value(self, attribute_name, sen, sheet, row, col):
        review_value = Review.OK
        if self._labels_gold_exists(sen):
            review_value = self.__set_gold_review_value(attribute_name, sen, sheet, row, col)
        elif attribute_name in sen[SF.GEN_ATTRIBUTES_ON_ANNOTATION]:
            self._color_gray(sheet, row, col + self.CONSTANTS.CATEGORY_OFFSET)
            self._color_gray(sheet, row, col + self.CONSTANTS.LABEL_OFFSET)
            self._color_gray(sheet, row, col + self.CONSTANTS.COUNT_OFFSET)
            self._color_gray(sheet, row, col + self.CONSTANTS.ANNOTATORS_OFFSET)
        sheet.cell(
            row=row, column=col + self.CONSTANTS.ATTRIBUTE_REVIEW_OFFSET
        ).value = review_value
    ...
    def __set_gold_review_value(self, attribute_name, sen, sheet, row, col):
        if attribute_name not in sen[SF.GOLD_ATTRIBUTES]:
            ...
            self._color_gold(sheet, row, col + self.CONSTANTS.CATEGORY_OFFSET)
            self._color_gold(sheet, row, col + self.CONSTANTS.LABEL_OFFSET)
            self._color_gold(sheet, row, col + self.CONSTANTS.COUNT_OFFSET)
            self._color_gold(sheet, row, col + self.CONSTANTS.ANNOTATORS_OFFSET)
            if attribute_name not in sen[SF.ANNOTATED_ATTRIBUTES]:
                return Review.MISSING
        return Review.OK

    def _add_validation(self, sheet):
        data_val = DataValidation(type="list", formula1=f"={self.CONSTANTS.ATTRIBUTE_NAMED_RANGE}")
        ...
        for row in range(self.CONSTANTS.FIRST_DATA_ROW, sheet.max_row + 1):
            self.__add_validation_for_row(sheet, row, data_val, review_val, sen_review_val)
    ...
    def __add_validation_for_row(self, sheet, row, data_val, review_val, sen_review_val):
        for cell in sheet[str(row)]:
            ...
            if self.__is_sentence_review_cell(col):
                self.__add_validation_for_sentence_review(sheet, row, col, sen_review_val)
            elif self._is_category_cell(col):
                self._add_validations_for_attribute(sheet, row, col, data_val)
                review_val.add(
