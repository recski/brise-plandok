...
class LabelReviewExcelGenerator(ExcelGenerator):
    ...
    def _fill_attribute(self, attribute, sen, sheet, col, row):
        ...
        ).value = ATTR_TO_CAT[attribute_name]
        sheet.cell(
    ...
    def __set_annotator_text(self, attribute_name, sen, sheet, row, col):
        ...
        sheet.cell(
            row=row, column=col + self.CONSTANTS.ANNOTATORS_OFFSET
        ).value = annotators
    ...
    def __add_validation_for_row(
        self, sheet, row, data_val, review_val, sen_review_val
    ):
    ...
    def __add_validation_for_sentence_review(
        self, sheet, row, col, sentence_review_val
    ):
