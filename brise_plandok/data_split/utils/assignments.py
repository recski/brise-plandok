...
import pandas
from brise_plandok.data_split.utils.constants import ASSIGNMENT_ADDITIONAL_HEADER, ASSIGNMENT_DF_HEADER, ASSIGNMENT_FILE, ASSIGNMENT_FILE_HEADER, DOC_HEADER
...
def fill_assignments_with_batch(docs, cycle, assignments, partition):
    ...
    for i, group in cycle.iterrows():
        mask = assignments[ASSIGNMENT_DF_HEADER[0]].isin(group.values)
...
def _calculate_sum_sentence_after_batch(group, assignments):
    for annotator in group:
        ...
        sentences_before = assignments.loc[annotator_mask, ASSIGNMENT_DF_HEADER[1]]
