...
CONDITIONS = {
    "PlanzeichenBBID", "WidmungID", "GebaeudeBautyp"}
...
class RuleExtractor(Extractor):
    ...
    def postprocess_fl(self, fl):
        ...
        new_fl = graph_to_isi(
            new_graph, ud=False, algebra='graph', root_id=root,
            preprocess=True)
    ...
    def attrs_to_rules(self, attr_tree_string, to_inherit=None):
        ...
        vals_to_quants = {}
        logging.debug(
            f'names: {names}, values: {values}, quants: {quants}'
            f'mods: {modalities}')
        ...
        while len(values_remaining) > 0 and len(names_to_match) > 0:
            ...
            all_pairs = [
                (n, v) for n in names_to_match for v in values_remaining]
    ...
    def run_on_parsed_sections(self, sections):
        ...
        for section in sections:
            ...
            for sen in section['sens']:
                ...
                if not gen_rules:
                    logging.debug(
                        f'no rules returned for sen {sen_id}, skipping')
                    continue
                    ...
                    "gen_attributes": [
                        {
                            ...
                            'type': attr['type'],
                            'value': attr['value']}
                        for rule in gen_rules for attr in rule['attributes']]}
